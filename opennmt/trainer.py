    ...
    def _gradient_accumulation(self, true_batches, normalization, total_stats,
        ...
        for k, batch in enumerate(true_batches):
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                #    dec_state.detach()
                logger.info(self.model.state["enc_outputs"])
                logger.info(self.model.state["dec_outputs"])
                logger.info(outputs)
                logger.info(tgt)
                logger.info(self.model.state["enc_outputs"].shape())
                logger.info(self.model.state["dec_outputs"].shape())
