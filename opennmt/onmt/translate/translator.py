...
class Translator(object):
    ...
    def _align_forward(self, batch, predictions):
        ...
        # (1) Encoder forward.
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch)[:4]
    ...
    def translate_batch(self, batch, src_vocabs, attn_debug, intervention_grad=None):
        ...
        with torch.no_grad():
            ...
            return self._translate_batch_with_strategy(batch, src_vocabs,
                                                       decode_strategy, intervention_grad=intervention_grad)
    ...
    def _translate_batch_with_strategy(
        ...
        if intervention_grad is not None:
            t1_norm = torch.norm(enc_states[0] )
            t2_norm = torch.norm(enc_states[1])
            t1 = enc_states[0] - intervention_grad[:256].reshape(2, 1, 128).clone()
            # t1 = (t1_norm/ torch.norm(t1)) * t1
            t2 = enc_states[1] - intervention_grad[256:].reshape(2, 1, 128).clone()
            #t2 = (t2_norm/ torch.norm(t2)) * t2
