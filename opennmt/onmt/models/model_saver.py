...
class ModelSaver(ModelSaverBase):
    ...
    def _save(self, step, model):
        ...
        logger.info("Storing information for DC.")
        
        state = {
            "vocab": model.vocab,
            "srcs": model.srcs,
            "tgts": model.tgts,
            "dec_outs": model.dec_outs,
            "preds": model.preds,
            "h_enc": model.h_enc,
            "c_enc": model.c_enc,
            #"h_dec": model.h_dec,
        ...
        if (step / 719) % 5 == 0 or step < (719 * 5):
            pickle.dump(state, open(f"{self.base_path}_step_{step}_hidden_states.pickle", 'wb'))
        model.srcs = []
        model.tgts = []
        model.dec_outs = []
        model.preds = []
