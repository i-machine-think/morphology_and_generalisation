...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        # Build the RNN.
        self.rnn = StackedLSTM(num_layers=num_layers, input_size=self._input_size,
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None, step=None,
        ...
        dec_state, dec_outs, attns, h_s, c_s, i_s, f_s, o_s = self._run_forward_pass(
        ...
        return dec_outs, attns, h_s, c_s, i_s, f_s, o_s
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None):
        ...
        for emb_t in emb.split(1):
            if len(input_feed.shape) == 2:
                input_feed = input_feed.unsqueeze(0)
            decoder_input = torch.cat([emb_t, input_feed], 2)
            rnn_output, dec_state, i_, f_, o_ = self.rnn(decoder_input, dec_state)
            #if self.attentional:
            ...
            decoder_output = rnn_output.squeeze(0)
            if self.context_gate is not None:
            ...
            h_s += [dec_state[0].detach().cpu().numpy()]
            c_s += [dec_state[1].detach().cpu().numpy()]
            i_s += [i_.detach().cpu().numpy()]
            f_s += [f_.detach().cpu().numpy()]
            o_s += [o_.detach().cpu().numpy()]
        ...
        return dec_state, dec_outs, attns, h_s, c_s, i_s, f_s, o_s
